...
--*/
{
    ...
    for ( CurProc = 0; CurProc < (ULONG) BasicInfo.NumberOfProcessors; CurProc++ ) {
        ...
        pcdTotalData.ProcessorTime +=
            pPCD->ProcessorTime = pProcessorInformation->IdleTime.QuadPart;
        pcdTotalData.UserTime +=
            pPCD->UserTime      = pProcessorInformation->UserTime.QuadPart;
        // kernel time is total kernel time less the time spent in the idle thread for that processor
        pcdTotalData.KernelTime +=
            pPCD->KernelTime    = pProcessorInformation->KernelTime.QuadPart - pPCD->ProcessorTime;
        ...
        pcdTotalData.InterruptTime +=
            pPCD->InterruptTime = pProcessorInformation->InterruptTime.QuadPart;

        pcdTotalData.DpcCountRate +=
            pPCD->DpcCountRate  = pThisProcessorInterruptInformation->DpcCount;

        pcdTotalData.DpcRate +=
            pPCD->DpcRate       = pThisProcessorInterruptInformation->DpcRate;
        pcdTotalData.DpcBypassRate +=
            pPCD->DpcBypassRate = pThisProcessorInterruptInformation->DpcBypassCount;
        pcdTotalData.ApcBypassRate +=
            pPCD->ApcBypassRate = pThisProcessorInterruptInformation->ApcBypassCount;
