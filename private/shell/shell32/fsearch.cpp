...
HKEY CFileSearchBand::GetBandRegKey( BOOL bCreateAlways )
{
    ...
    if( MakeBandKey( szKey, ARRAYSIZE(szKey) ) > 0 )
    {
        ...
        {
            DWORD dwDisp;
            if( ERROR_SUCCESS != RegCreateKeyEx( HKEY_CURRENT_USER, szKey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hkey, &dwDisp ) )
                hkey = NULL;
        }
        else
        {
            if( ERROR_SUCCESS != RegOpenKeyEx( HKEY_CURRENT_USER, szKey, 0, KEY_ALL_ACCESS, &hkey ) )
                hkey = NULL;
        }
    }
    ...
    BOOL bDefaultFocusCtl )
{
    ...
    if( SUCCEEDED( (hr = CoCreateInstance( CLSID_SearchAssistantOC, NULL, CLSCTX_INPROC_SERVER, IID_IObjectWithSite, (void**)&pows )) ) )
    {
